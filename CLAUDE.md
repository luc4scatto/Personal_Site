...
- **Player: "next set" su iOS - DA RIVEDERE, non chiuso.** Su iPad Chrome il cambio set non riparte da solo, sul desktop sì. Causa: `widget.load()` sostituisce il documento dell'iframe, che non ha user activation propria, quindi iOS ignora `auto_play`; e `onReady` arriva in un task successivo, quando la gesture del click è già scaduta. Fatto finora (`src/setPlayer.js`, `engineLoad()`): `pendingPlay` armato anche nel ramo di riuso, `askPlay()` chiamato **dentro lo stesso click** subito dopo `load()`, `curTime`/`scrubbing` azzerati in `load()`, loop rAF riacceso dopo `getDuration`, e un watchdog da 3s (`PLAY_WAIT`) che riporta il transport in pausa con la nota "Tap play to start this set" invece di lasciare la barra congelata sul silenzio. Non basta ancora. Prossimo tentativo, quello che ha davvero probabilità di funzionare: **non ricaricare l'iframe**. Creare una playlist SoundCloud con i 4 set, caricarla una volta sola e cambiare traccia con `widget.next()` / `widget.skip(i)`, che restano nello stesso documento e quindi nello stesso sblocco audio. Serve che Luca crei la playlist su SoundCloud; `content.djSets` diventa un indice dentro quella playlist invece di 4 URL di traccia.
...
Resa su iPad Pro 11" (Chrome iOS): risolti il framerate generale, il marquee che si inceppava, la prima riga dell'h1 sfocata e le cartelle che compenetravano la card aperta - vedi i tre gotcha sul budget GPU, sul blur per-cartella e su `pickPull()`. Resta il "next set" del player che non riparte da solo (vedi TODO, primo punto).
